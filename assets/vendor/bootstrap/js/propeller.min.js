$(document).ready(function() { /* Textfield */
    ...
    $('.pmd-textfield input.form-control').each(function() { if ($(this).val() != "") { $(this).closest('.pmd-textfield').addClass("pmd-textfield-floating-label-completed"); } });
    $(".pmd-textfield input.form-control").on('change', function(e) { if ($(this).val() != "") { $(this).closest('.pmd-textfield').addClass("pmd-textfield-floating-label-completed"); } });
    $("body").on("focus", ".pmd-textfield .form-control", function() { $(this).closest('.pmd-textfield').addClass("pmd-textfield-floating-label-active pmd-textfield-floating-label-completed"); });
    $("body").on("focusout", ".pmd-textfield .form-control", function() {
        if ($(this).val() === "") { $(this).closest('.pmd-textfield').removeClass("pmd-textfield-floating-label-completed"); }
    ...
    $(".pmd-checkbox-ripple-effect").on('mousedown', function(e) {
        var rippler = $(this);
        $('.ink').remove();
        if (rippler.find(".ink").length == 0) { rippler.append('<span class="ink"></span>'); }
        ...
        setTimeout(function() { ink.remove(); }, 1500);
    });
        ...
        if ($(window).width() < 767) {
            ...
            dropdown.on('show.bs.dropdown', function(e) {
                ...
                if ($dataSidebar == 'true') {
                    ...
                    } else if (that.hasClass('pmd-dropdown-menu-top-left')) { dcdmbg.addClass('pmd-dropdown-menu-bg-bottom-left'); } else if (that.hasClass('pmd-dropdown-menu-top-right')) {
            ...
            dropdown.on('hide.bs.dropdown', function(e) {
                ...
                var dcdmc = that.closest('.pmd-dropdown-menu-container');
                var dcdmbg = dcdmc.find('.pmd-dropdown-menu-bg');
                if ($dataSidebar == 'true') { that.first().stop(true, true).slideUp(300); } else if (dropdowncenter) {
                    $('.dropdown-menu').removeAttr('style');
                    ...
                    if (that.hasClass('dropdown-menu-right')) { that.css("clip", "rect(0 " + w + "px 0 " + w + "px)"); } else if (that.hasClass('pmd-dropdown-menu-top-right')) { that.css("clip", "rect(0 " + w + "px 0 " + w + "px)"); }
                }
            });
        } else {
            $('.dropdown-menu').removeAttr('style');
            ...
            dropdown.on('show.bs.dropdown', function(e) {
                ...
                var dcdmc = that.closest('.pmd-dropdown-menu-container');
                var dcdmbg = dcdmc.find('.pmd-dropdown-menu-bg');
                if (hassidebar) { that.first().stop(true, true).slideDown(); } else if (dropdowncenter) {
                    if (!$(this).parents().hasClass("pmd-sidebar")) { $('.dropdown-menu').removeAttr('style'); }
                    ...
                    if (that.hasClass('dropdown-menu-right')) {
                        ...
                        dcdmbg.addClass('pmd-dropdown-menu-bg-bottom-right');
                        dcdmc.css({ "right": "0", "left": "auto" })
                    } else { that.css("clip", "rect(0 " + w + "px " + h + "px 0)"); }
                }
                this.closable = false;
            });
            dropdown.on('click', function(e) {
                ...
                } else if ($(this).parents("aside").hasClass("pmd-sidebar")) { $('.dropdown-menu').slideUp(300); }
                this.closable = true;
            });
            dropdown.off('hide.bs.dropdown');
            dropdown.on('hide.bs.dropdown', function(e) {
                if ($(this).parents("aside").hasClass("pmd-sidebar")) { return this.closable; } else {
                    $dataSidebar = $(this).find('.dropdown-toggle').attr("data-sidebar");
                    var hassidebar = $(this).closest('.pmd-sidebar').hasClass('pmd-sidebar');
                    var dropdowncenter = $(this).find('.dropdown-menu').hasClass('pmd-dropdown-menu-center');
                    var that = $(this).find('.dropdown-menu');
                    ...
                    var dcdmc = that.closest('.pmd-dropdown-menu-container');
                    var dcdmbg = dcdmc.find('.pmd-dropdown-menu-bg');
                    if (hassidebar) { that.first().stop(true, true).slideUp(300); } else if (dropdowncenter) {
                        if (!$(this).parents().hasClass("pmd-sidebar")) { $('.dropdown-menu').removeAttr('style'); }
                        that.first().stop(true, true).slideUp(300);
                    } else {
                        if (!$(this).parents().hasClass("pmd-sidebar")) {
    ...
    pmdsidebardropdown();
    $(window).resize(function() { pmdsidebardropdown(); });
    ...
    $(function() {
        ...
        $('a[data-toggle="collapse"]').on('click', function() {
            ...
            if (expandale == 'true') { if ($(objectID).hasClass('in')) { $(objectID).collapse('hide'); } else { $(objectID).collapse('show'); } }
            ...
            if ($expandable == "false") {
                if ($expanded == "true") { $("#" + $current + " .active").removeClass("active"); } else {
                    $("#" + $current + " .active").removeClass("active");
                    $(this).parents('.panel').addClass("active");
                }
            }
            if ($expandable == "true") { if ($expanded == "true") { $(this).parents('.panel').addClass("active"); } else { $(this).parents('.panel').removeClass("active"); } }
        });
...
(function($) {
    $.fn.pmdTab = function(options) {
        ...
        return this.each(function() {
            ...
            var widthOfHidden = function() { return (widthOfList() - ($this.outerWidth())) };
            $this.find('ul.nav-tabs').width(widthOfList());
            var getLeftPosi = function() { return $this.find('ul.nav-tabs').position().left; };
            ...
            var sliderLoad = function() {
                var $slider = $this.find('.pmd-tab-active-bar'),
                    ...
                    $wrapperLeft = $this.offset().left;
                $sliderLeft = $isX - $wrapperLeft, $wrapPadLeft = $this.css("padding-left"), $paddingAmount = parseInt($wrapPadLeft, 10);
                $finalPossion = $wrapperLeft + $paddingAmount - $navX + $isX - $wrapperLeft;
                if ($navX < $paddingAmount + $wrapperLeft) { $slider.width($sliderActive.width() + "px").css("left", $finalPossion + "px"); } else { $slider.width($sliderActive.width() + "px").css("left", $sliderLeft + "px"); }
            ...
            sliderLoad();
            $(window).on("load resize", function() { sliderLoad(); });
            $this.find('.pmd-tabs-scroll-right').click(function() {
                ...
                if (finalTab == 0) {
                    ...
                    $(this).fadeOut('slow');
                } else { $wrapper.animate({ scrollLeft: '+=' + $tabSet }); }
            ...
            $this.find('.pmd-tabs-scroll-left').click(function() {
                ...
                if (finalTab == 0) {
                    ...
                    $wrapper.animate({ scrollLeft: '-=' + NewScrollAmount });
                    $(this).fadeOut('slow');
                } else { $wrapper.animate({ scrollLeft: '-=' + $tabSetLeft }); }
...
$(document).ready(function() {
    ...
    $('.pmd-topbar-toggle').on('click', function(e) { pmdtopbartoggle.toggleClass('pmd-sidebar-open'); });
    $('.topbar-close').on('click', function() { pmdtopbartoggle.removeClass('pmd-sidebar-open'); });
        ...
        if (value && typeof value.test === "function") {
            for (var i = 0, l = this.length; i < l; i++) {
                ...
                if (elem.nodeType === 1 && elem.className) {
                    ...
                    for (var n = classNames.length; n--;) { if (value.test(classNames[n])) { classNames.splice(n, 1); } }
